...
In test 1, we basically just check if the corresponding lower bound is less than upper bound, and the function passes the test as long as the output is TRUE. Then we tried to plot our upper/lower bound using previous function we defined: $make_z$.We use a logconcave function $f(x)=x^2$, and it turns out the plot is what we had expected.
...
Since the histogram of sample_upper_bound function matchs the trend of $f(x) = -x^2$, so it is obvious that our sample_upper_bound function works as we expected.
...
# Basically, we have our candidates and cand_filtered, and the function does what it is suppose to do.
cand
cand_filtered
...
# When accepted point is 'NA'
accepted <- NA
update_sample(cand, accepted, update,  f, upper_bound)

# In this case, the update_sample_function returns the index of the last accepted point, and it returns 1 if the point satisfies the test "flag" and we accepted 
no point. It returns 0 if the point does not satisfy "flag" and we accepted no point. According to the test, it works correctly with specific output.
